...
void get_incompat_flag(struct cmfs_super_block *sb, char *buf, size_t count);
void get_tunefs_flag(struct cmfs_super_block *sb, char *buf, size_t count);
void get_compat_flag(struct cmfs_super_block *sb, char *buf, size_t count);
void get_rocompat_flag(struct cmfs_super_block *sb, char *buf, size_t count);
void get_cluster_info_flag(struct cmfs_super_block *sb, char *buf,
			   size_t count);
...
errcode_t string_to_inode(cmfs_filesys *fs, uint64_t root_blkno,
			  uint64_t cwd_blkno, char *str, uint64_t *blkno);
errcode_t dump_file(cmfs_filesys *fs, uint64_t ino, int fd, char *out_file,
		    int preserve);
errcode_t read_whole_file(cmfs_filesys *fs, uint64_t ino, char **buf,
			  uint32_t *buflen);
void inode_perms_to_str(uint16_t mode, char *str, int len);
void inode_time_to_str(uint64_t mtime, char *str, int len);
errcode_t rdump_inode(cmfs_filesys *fs, uint64_t blkno, const char *name,
		      const char *dumproot, int verbose);
void crunch_strsplit(char **args);
void find_max_contig_free_bits(struct cmfs_group_desc *gd, int *max_contig_free_bits);
...
errcode_t traverse_extents(cmfs_filesys *fs, struct cmfs_extent_list *el,
			   FILE *out);
errcode_t traverse_chains(cmfs_filesys *fs, struct cmfs_chain_list *cl,
			  FILE *out);
