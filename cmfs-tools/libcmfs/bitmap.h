 ...
 * (Copied and modified from ocfs2-tools/libocfs2/bitmap.h)
 *
...
struct cmfs_bitmap_operations {
	errcode_t (*set_bit)(cmfs_bitmap *bitmap, uint64_t bit,
			     int *oldval);
	errcode_t (*clear_bit)(cmfs_bitmap *bitmap, uint64_t bit,
			       int *oldval);
	errcode_t (*test_bit)(cmfs_bitmap *bitmap, uint64_t bit,
			      int *val);
	errcode_t (*find_next_set)(cmfs_bitmap *bitmap,
				   uint64_t start, 
				   uint64_t *found);
	errcode_t (*find_next_clear)(cmfs_bitmap *bitmap,
				     uint64_t start, 
				     uint64_t *found);
	int (*merge_region)(cmfs_bitmap *bitmap,
			    struct cmfs_bitmap_region *prev,
			    struct cmfs_bitmap_region *next);
	errcode_t (*read_bitmap)(cmfs_bitmap *bitmap);
	errcode_t (*write_bitmap)(cmfs_bitmap *bitmap);
	void (*destroy_notify)(cmfs_bitmap *bitmap);
	void (*bit_change_notify)(cmfs_bitmap *bitmap,
				  struct cmfs_bitmap_region *br,
				  uint64_t bitno,
				  int new_val);
	errcode_t (*alloc_range)(cmfs_bitmap *bitmap, uint64_t min_len,
				 uint64_t len, uint64_t *first_bit,
				 uint64_t *bits_found);
	errcode_t (*clear_range)(cmfs_bitmap *bitmap, uint64_t len, 
				 uint64_t first_bit);
};
